...
class PolicyNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            "checkpoint_path",
            "/ppo_ws/src/ppo_controller/models/"
            "random_1_scan_collision_progress+alive_velocity+steeringangle_10_v0_actor_params.safetensors",
        )
        checkpoint_path = self.get_parameter("checkpoint_path").value

        weights = load_file(checkpoint_path)

        self.obs_dim = weights["params,Dense_0,kernel"].shape[0]
        self.get_logger().info(
            f"Loaded checkpoint: {checkpoint_path} (obs_dim={self.obs_dim})"
        )

        self.policy = Actor(self.obs_dim)
    ...
    def obs_callback(self, msg):
        ...
        if len(state) != self.obs_dim:
            # Fail loud rather than silently feeding a wrong-length vector
            # into the network (e.g. scan disabled on the observation side,
            # or scan params out of sync with what this checkpoint expects).
            if not self._warned_mismatch:
                self.get_logger().error(
                    f"Observation length {len(state)} != checkpoint obs_dim "
                    f"{self.obs_dim}. Skipping inference until this is fixed."
                )
